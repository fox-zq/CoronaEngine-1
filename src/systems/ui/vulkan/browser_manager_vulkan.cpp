...
void BrowserManager::destroy_tab_texture(BrowserTab* tab) {
    if (!tab || tab->texture_id == VK_NULL_HANDLE) {
        return;
    }
...
VkDescriptorSet BrowserManager::create_browser_texture(int width, int height) {
    ...
    owned_images_[descriptor] = {image, image_memory, image_view, sampler,
...
void BrowserManager::update_texture(int tab_id) {
    ...
    VkDeviceSize image_size = tab->pixel_buffer.size();
    ...
    vkCreateBuffer(device, &buf_info, nullptr, &staging_buffer);
    VkMemoryRequirements mem_reqs;
    ...
    memcpy(mapped, tab->pixel_buffer.data(), image_size);
    ...
    vkCmdCopyBufferToImage(cmd_buf, staging_buffer, found->image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);
